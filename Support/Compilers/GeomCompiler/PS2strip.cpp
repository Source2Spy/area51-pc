...
xhandle ps2_strip::GetBestRun( s32& iD1, s32& iD2, s32 Size )
{
    ...
    {        
        ...
        for( s32 j=0; j<s_nDir; j++ )
        {
            ...
            // Get how many verts added
            if( RunScore > 0 ) RunScore = RunScore + 2;
            ...
            else if( RunScore == (Size-2) || RunScore == (Size-1) ) 
            {
                // This will be a really bad thing
                ExtraScore -= 1000000;
            }
            else if( RunScore > Size )
            {
...
void ps2_strip::BuildStrip( xhandle hTri, s32 iD1, s32 iD2, s32 Count )
{
    ...
    // Make sure to pad to chache size
    // This is a really bad thing since actually add verts that do nothing.
    if( m_lTriangle.GetCount() > 0 && (Count <= 2) )
    {
        for( ;Count; Count--)
            m_Index.Append() = m_lTriangle[0].iV[iD1] | (1<<30);
    }
}

//==============================================================================

void ps2_strip::SanityCheck( void )
{
    for( s32 i=0; i<GetIndexCount(); i += m_CacheSize )
    {
        if( IsIndexNewStrip(i) == FALSE || IsIndexNewStrip(i+1) == FALSE ) 
            x_throw( "Striper internal error. (Didn't start a new strip in a cache boundary" );
    }
}
...
void ps2_strip::End( void )
{
    ...
    //
    // Do an integrity check before saying that we are done
    //
    SanityCheck();
}
